job('NodeJS-dev-api') {
    ...
    scm {
        ...
        credentials('Ajna-source-access')
        branch('CDEV')  
            }
    wrappers {
	...
	--region us-east-1 \
    --application-name ajna-cdev \
    --deployment-config-name CodeDeployDefault.AllAtOnce \
    --deployment-group-name ajna-cdev-api\
    --description "Ajna dev Api deployment" \
    --s3-location bucket=ajnabuildartifact,bundleType=zip,key=$JOB_NAME-1.2.$BUILD_ID.zip")
    }
}
